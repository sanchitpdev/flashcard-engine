    ...
    <div style={{
      ...
      <div className="animate-in" style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: '1.6rem', marginBottom: 4 }}>Start learning</h1>
          <p style={{ color: 'var(--text-sub)', fontSize: '0.9rem' }}>Create your free account</p>
        </div>
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button type="submit" className="btn btn-primary" disabled={loading}
            ...
            {loading ? <Spinner size={18} color="var(--navy)" /> : 'Create Account →'}
          </button>
        </form>

        <p style={{ textAlign:'center', marginTop:20, fontSize:'0.9rem', color:'var(--text-sub)' }}>
          Already have an account?{' '}
          <Link to="/login" style={{ color:'var(--primary-dark)', fontWeight:700 }}>Sign in</Link>
        </p>
      </div>
    </div>
  )
